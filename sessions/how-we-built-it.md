...
architecture では、Engine は schedule または manual trigger で起動し、multi-tenant orchestration / distributed task queue / LangSmith Deployment / Deep Agents / LangSmith Sandboxes を使う。trace は condensed / summarized version を入力にし、必要に応じて深掘りする。source code も optional に接続され、diagnosis と PR generation に使われる。
...
- Engine は Deep Agents、LangSmith Sandboxes、LangSmith Deployment など複数の LangChain 製品で構成される。
